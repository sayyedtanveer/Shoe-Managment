...
export class OcrService {
    async enqueueOcr(filePath: string, shopId: string) {
        const job = await ocrQueue.add('ocr-upload', { filePath, shopId }, {
            removeOnComplete: 50,
            removeOnFail: 50,
        });
        return job.id;
    ...
    async getJobStatus(id: string, shopId: string) {
        const job = await ocrQueue.getJob(id);
        if (!job) return null;
        if (job.data.shopId !== shopId) return null;
