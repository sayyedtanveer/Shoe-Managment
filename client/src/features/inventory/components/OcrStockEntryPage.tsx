...
function parseLinesToRows(extractedLines: string[], activeMapping: Record<string, string>) {
    return extractedLines
        ...
        .map((cols) => {
            ...
            cols.forEach((value, colIndex) => {
                const field = activeMapping[String(colIndex)] ?? 'ignore';
...
export function OcrStockEntryPage() {
        ...
        const poll = async () => {
            ...
            if (payload.state === 'completed') {
                ...
                const defaultMapping = Object.fromEntries(
                    detectedHeaders.map((_, index) => [String(index), index === 0 ? 'name' : index === 1 ? 'quantity' : index === 2 ? 'price' : 'ignore'])
                );
                setMapping(defaultMapping);
                setRows(parseLinesToRows(extractedLines, defaultMapping));
                setLoading(false);
                return;
            }
    ...
    const updateMapping = (columnIndex: number, value: string) => {
        ...
        setRows(parseLinesToRows(lines, next));
    };
